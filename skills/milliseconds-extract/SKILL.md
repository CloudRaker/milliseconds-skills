...
The response keeps its shape: `data` in your schema, with `null` for every field the image does not carry.
...
`texts` with `image` is a 400 (`image_with_texts`); one image per request. Billing on images: the body without the base64, plus 196, plus 2,000 / 4,000 / 8,000 by tier. Read `x-input-tokens`. Measured on receipts: 74 % on flat fields, 0.82 F1 on line items. An image schema accepts at most 5 fields; a larger schema is a 400 (`image_schema_too_large`). Count every leaf property: nested object properties count, and an array-of-objects item counts its properties once each, so `{merchant, total, items[{name, price}]}` is 4 fields. Extract on an image is a generative pass of 2.5 to 3.5 seconds, built for a few fields off one page, never for full-page transcription. Below the cap keep the pixels: the text route lost 20 F1 points on line items. Above it, split the schema across calls. For whole pages and long documents, send the user to the CloudRaker paperwork API at https://cloudraker.com: it parses, extracts and grounds full documents with no field cap.
