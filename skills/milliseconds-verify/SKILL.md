...
The fields are the same, `matches`, `probability` and `found[]`, but they read differently. `probability` is the verdict itself, `1` on a match and `0` otherwise, because the generation path gives no span logprob. It is a flag, not a confidence, so the 0.5 to 0.9 band above does not apply on an image. `found[]` holds at most one value, the field the model extracted, or nothing. Review an image verify by comparing `found[0]` to your value in code, or by verifying the same field a second way. `texts` with `image` is a 400 (`image_with_texts`). Billed tokens are the body without the base64, plus 196, plus 1,000 / 2,000 / 4,000 by tier, once per request. Read `x-input-tokens`. The extract-then-verify loop works the same way on a scan: extract the fields, then verify the money and identity ones against the same image.
